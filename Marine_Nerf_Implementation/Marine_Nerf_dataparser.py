...
class Marine_Nerf_Dataparser(DataParser):
    ...
    def generate_dataparser_outputs(self, split="train"):
        ...
        for frame in meta["frames"]:
            ...
            if "depth_file_path" in frame:
                depth_filenames = Path(frame["depth_path"])
            ...
            if "sky_segmentation_mask" in frame:
                sky_masks_filenames = Path(frame["sky_segmentation_mask"])
                sky_seg_fname = self._get_fname(
                    sky_masks_filenames,
                    data_dir,
                    downsample_folder_prefix="sky_semgmentations_"
                )
            ...
            sky_masks_filenames = [sky_masks_filenames[i] for i in indices] if len(sky_masks_filenames) > 0 else []
